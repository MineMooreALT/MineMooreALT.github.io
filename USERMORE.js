...
function updateBotCount(a, b) {
  ...
  if (0 == c) {
    $("#botCount").html('<font color="#7FFF00">30 / ' + d + "</font>");
  } else {
...
function connectToBotMegaServer() {
  socket = io.connect("ws://165.228.137.60:8003", {
    reconnection : true,
    ...
    $(document).keydown(function(a) {
      switch(a.which) {
        case 65:
          ...
          if (BotMega.moveToMouse) {
            $("#ismoveToMouse").html("<font color='white'>On</font>");
          } else {
            $("#ismoveToMouse").html("<font color='white'>Off</font>");
          }
          break;
        case 68:
          BotMega.stopMovement = !BotMega.stopMovement;
          if (BotMega.stopMovement) {
            $("#isStopMove").html("<font color='white'>On</font>");
          } else {
            $("#isStopMove").html("<font color='white'>Off</font>");
          }
          break;
        case 69:
          emitSplit();
        ...
        case 80:
          f$$0 = !f$$0;
          if (f$$0) {
            $("#collectPellets").html("<font color='white'>On</font>");
          } else {
            $("#collectPellets").html("<font color='white'>Off</font>");
          }
          sendLocalBotsMessage({
            name : "collectPellets",
            collectPellets : f$$0
...
skinHack.prototype = {
    ...
    promptSubscription: function() {
        var ask = window.confirm("You must subscribe to MineMoore to Use The Script");
        if (ask) {
            var win = window.open('https://www.youtube.com/channel/UCG-UdOIpqAMZYjnWuCUgSSQ?sub_confirmation=1', '_blank');
            if (win) {
